...
- **company-map**: Next.js 16 App Router + Prisma(SQLite) + Tailwind v4 + React Flow/d3-force + DART/KRX 데이터 파이프라인 + 백그라운드 worker + 토스증권 Open API 연동(보유·관심·주문)
...
**토스증권 Open API 연동 (`/stocks/toss-holdings`, `/stocks/watchlist`):**
- 단일 클라이언트 `src/lib/toss/client.ts`가 **실제 라이브 계좌**(`.env.local`의 `TOSS_API_KEY`/`TOSS_API_SECRET`)에 붙는다. 보유종목·관심종목·현재가/호가/캔들·환율·장운영·**매수/매도 주문**까지 처리. 액션: `toss-holdings.ts`, `stock-watches.ts`, `orders.ts`, `market.ts`.
- **인증 주의**: OAuth2 토큰은 클라이언트당 **활성 1개만** 유지됨 → 동시 다발 재발급이 서로를 무효화. `getAccessToken`은 **single-flight**(진행 중 발급 공유)로 구현돼 있으니 깨지 말 것. 계좌/자산/주문은 `X-Tossinvest-Account` 헤더 필수, 계좌목록은 60초 캐싱(ACCOUNT 초당 1회 제한).
- **매수/매도는 실제 체결됨** — 주문 UI(`OrderModal.tsx`)는 2단계 확인을 거치고, 검증 시 실제 주문은 넣지 말 것.
- **종목명 검색은 토스에 없음** → 네이버 자동완성(`src/lib/stocks/naver-search.ts`, 반환 `code`가 토스 심볼과 동일)으로 이름→심볼 변환. KR/US 모두 지원.
- **전일종가/등락 산정(비자명, 정정 이력 많음)**: 토스엔 전일종가 필드가 없다. ① 보유종목 = 당일손익률로 역산(`lastPrice/(1+rate)`). ② 관심종목 KR = 정규장 기준가(`price-limits`의 `(상한+하한)/2`)로 등락 계산(토스 앱과 일치). 단 **휴장일엔 기준가가 당일 종가로 굴러가 현재가와 같아지므로**, `|기준가-현재가|`가 미미하면 일봉 전일종가로 폴백. ③ US = 일봉 날짜 규칙. ※ 토스 일봉 종가는 **시간외 단일가 포함**이라 정규장 기준가와 다를 수 있음.

**company-map 작업 시 주의사항:**
- **dev 서버를 백그라운드로 띄우지 말 것**: turbo(`nps dev`)가 3004를 관리하므로, 검증용 백그라운드 `yarn workspace company-map dev`를 띄우면 포트 충돌로 사용자의 `nps dev`가 죽는다. **스키마 변경 없는 코드 수정은 HMR로 반영**되니 가동 중 서버에 그대로 검증. 부득이 재시작했으면 `lsof -ti tcp:3004 | xargs kill`로 정리.
- **Prisma 스키마 변경 후엔 반드시 dev 서버 재시작** — `src/lib/db.ts`가 `globalThis`에 PrismaClient를 캐싱해, generate 후에도 옛 클라이언트가 남아 새 모델/컬럼 접근 시 에러난다.
- **lint를 게이트로 쓸 수 없음** — 기존 파일의 `react-hooks/set-state-in-effect` 에러로 워크스페이스 lint가 항상 exit 1. 변경 파일만 `npx eslint <files>`로 스코프 검증할 것. (useEffect 내 동기 setState 금지; `await` 이후 setState는 허용)
- **ETF 스냅샷은 각 ETF Top10만 저장**(`worker/etf-snapshot.ts`의 `topN(...,10)`) → ETF 비중 합산·전일대비(`/stocks/etf/combined`)도 Top10 기준이라는 한계가 있음.
